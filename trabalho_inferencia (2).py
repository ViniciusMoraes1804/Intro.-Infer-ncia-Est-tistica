# -*- coding: utf-8 -*-
"""trabalho_inferencia.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1XCI7HsAEqmX9sCt7_QC0HjCkebDS4xIW
"""
...
tab1 = pd.crosstab(index=df['social_platform_preference'], columns='count')
tab1.plot.pie(y='count')
...
#Vamos ontar quantos grupos rejeitaram a normalidade
...
for coluna in ['work_hours_per_day', 'actual_productivity_score']:
    ...
    if(p > 0.05):
      print(f'\n{coluna}: Aceito a normalidade (p-valor = {p})')
    else:
...
if p < 0.05:
    print("Há evidência significativa de correlação linear.")
else:
    print("Não há evidência significativa de correlação linear.")
...
for genero in amostra['gender'].unique():
    grupo = amostra[amostra['gender'] == genero]['work_hours_per_day']
    stat, p = stats.shapiro(grupo)
    if(p > 0.05):
      print(f'\n{genero}: Aceito a normalidade (p-valor = {p})')
    else:
...
grupos = [amostra[amostra['gender'] == genero]['work_hours_per_day'] for genero in amostra['gender'].unique()]
stat, p = stats.levene(*grupos)
if(p > 0.05):
  print(f'Aceito a homocedasticidade (p-valor = {p})')
else:
  print(f'Rejeito a homocedasticidade (p-valor = {p})')

# Visualizar os desvios-padrões
desvios = amostra.groupby('gender')['work_hours_per_day'].std()
...
Como os grupos de 'gender' não rejeitaram a normalidade e a homocedasticidade de 'work_hours_per_day', utilizaremos o teste de análise de variância (ANOVA) para verificar a relação entre essas duas variáveis.
...
if p > 0.05:
    print(f"Não há diferença significativa entre as médias dos grupos. (p-valor = {p})")
else:
    print(f"Pelo menos uma das médias é significamente diferente das demais. (p-valor = {p})")

# Visualização do teste
plt.figure(figsize=(8, 5))
sns.boxplot(data=amostra, x='gender', y='work_hours_per_day')
plt.title("Quantidade de horas trabalhadas por gênero")
plt.ylabel("Quantidade de horas trabalhadas")
plt.xlabel("Gênero")
plt.show()
...
model = smf.ols('work_hours_per_day ~ C(gender)', data=amostra).fit()
anova_lm(model, typ=2)

"""### **Conclusão final**

A fim de verificar a relação entre o gênero e a quantidade de horas trabalhadas, primeiro tivemos que verificar a normalidade (teste de Shapiro_Wilk) e a homocedasticidade (teste de Levene) das variáveis, para descobrir se seria possível aplicar o método ANOVA ou não. Como nenhum dos dois foi rejeitado, aplicamos a Análise de Variância. Nela, foi encontrado um p-valor maior do que 0.05, de forma a não rejeitarmos H0. Isso significa que não há diferenças significativas na quantidade de horas trabalhadas entre homens, mulheres e outros (todos trabalham, aproximadamente, por um mesmo período de tempo).
"""